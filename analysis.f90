...
!-----------------!
 ...
 do i=1,ntime                                            !Start propagation
    ...
    call calcnorm(psi, npoints, dx, t, dt)               !Calculates the norm of the wavefunction
    call calcenergy(psi, npoints, dx, t, dt, pot, kin)   !Calculates E_pot, E_kin and E_total
    call autocorrelation(psi, psi0, npoints, dx, t, dt)  !Calculates the survival probability
...
!-------------------------------------------!
 subroutine calcnorm(psi, npoints, dx, t, dt)
!-------------------------------------------!
 use parameters
 implicit none
 ...
 integer :: npoints, i
 real(8) :: dx, t, dt, norm
 complex(8), dimension(npoints) :: psi

 norm = 0.0d0

 ! Calculate the norm integral
 do i = 1, npoints
   norm = norm + conjg(psi(i)) * psi(i) * dx
 end do

 ! Write headers and the norm to a file
 if (t == dt) then
   write(12, *) '# time, norm'
 endif
 write(12, '(F12.2, 2X, F8.4)') t/fs2au, norm

 end subroutine calcnorm
!-----------------------!

!-------------------------------------------------------!
 subroutine calcenergy(psi, npoints, dx, t, dt, pot, kin)
!-------------------------------------------------------!
 use parameters
 implicit none

 integer :: npoints, i
 real(8) :: dx, t, dt, E_pot, E_kin, E_total
 real(8), dimension(npoints) :: pot, kin
 complex(8), dimension(npoints) :: psi, psi_momentum
 E_pot = 0.0d0
 E_kin = 0.0d0

! Calculate potential energy
 do i = 1, npoints
   E_pot = E_pot + conjg(psi(i)) * pot(i) * psi(i) * dx
 end do

 ! Calculate kinetic energy
 psi_momentum = psi
 call fourier(1, npoints, psi_momentum)
 psi_momentum = kin * psi_momentum
 call fourier(-1, npoints, psi_momentum)

 do i = 1, npoints
   E_kin = E_kin + conjg(psi(i)) * psi_momentum(i) * dx
 end do

 ! Total energy
 E_total = E_pot + E_kin

 ! Write headers and the energies to a file
 if (t == dt) then
   write(13, *) '# time, E_pot, E_kin, E_total'
 endif
 write(13, '(F12.2, 2X, F12.8, 2X, F12.8, 2X, F12.8)') t/fs2au, E_pot, E_kin, E_total

 end subroutine calcenergy
!-------------------------!

!--------------------------------------------------------!
 subroutine autocorrelation(psi, psi0, npoints, dx, t, dt)
!--------------------------------------------------------!
 use parameters
 implicit none
 
 integer :: npoints, i
 real(8) :: dx, t, dt, surv_prob
 complex(8), dimension(npoints) :: psi, psi0
 complex(8) :: autocorr
 ...
 do i = 1, npoints
   autocorr = autocorr + conjg(psi0(i)) * psi(i) * dx
 end do
 surv_prob = abs(autocorr)**2

 ! Write headers and the surv_prob to a file
 if (t == dt) then
   write(14, *) '# time, surv_prob'
 endif
 write(14, '(F12.2, 2X, F8.4)') t/fs2au, surv_prob
...
!---------------------!
 ...
 call fourier(0, ntime, autocorr)

 call fourier(?, ntime, autocorr)

 plot eigenvelues from autocorr vs step 2pi/ntime*dt
